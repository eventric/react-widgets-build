...
function (_React$Component) {
  ...
  _proto.renderCalendar = function renderCalendar() {
    ...
    calendarProps.baseDateTime = this.props.baseDateTime;
  ...
  initializer: function initializer() {
    ...
    return function (selectedDate) {
      var _this7$props = _this7.props,
          timeZone = _this7$props.timeZone,
          time = _this7$props.time,
          value = _this7$props.value,
          date = _this7$props.date;
      var format = getFormat(_this7.props);
      var momentDate = selectedDate && (0, _momentTimezone.default)(selectedDate);
      var dateTime = time && date ? (0, _momentTimezone.default)(value || undefined).tz(timeZone).year(momentDate.year()).month(momentDate.month()).date(momentDate.date()).toDate() : selectedDate;
      var dateStr = formatDate(dateTime, format, _this7.props.culture, _this7.props.timeZone, !_this7.props.time);

      _this7.close();

      (0, _widgetHelpers.notify)(_this7.props.onSelect, [dateTime, dateStr]);

      _this7.handleChange(dateTime, dateStr);
  ...
  initializer: function initializer() {
    ...
    return function (datum) {
      ...
      let dateTime
      if (_this8.props.value) {
        dateTime = datum.date
      } else {
        var momentDate = (0, _momentTimezone.default).utc(_this8.props.baseDateTime || undefined)
        dateTime = (0, _momentTimezone.default)(datum.date).tz(_this8.props.timeZone).year(momentDate.year()).month(momentDate.month()).date(momentDate.date()).toDate()
      }
      var dateStr = formatDate(dateTime, format, _this8.props.culture, _this8.props.timeZone, !_this8.props.time)

      _this8.close();

      (0, _widgetHelpers.notify)(_this8.props.onSelect, [dateTime, dateStr]);

      _this8.handleChange(dateTime, dateStr);
